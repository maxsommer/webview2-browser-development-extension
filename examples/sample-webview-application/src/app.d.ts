 ...
 * prototype are run in the JavaScript engine of the current document. Additionally unknown property
...
export interface HostObjectsOptions {
	 ...
	 * or regular expression in this array. Setting this value to /Async$/ will make unknown method
 ...
 * prototype are run locally. Additionally unknown property or method in the
...
interface WebViewEventListenerObject {
	handleEvent(object: Event & { data?: unknown }): void;
}

interface WebViewEventListener {
	(evt: Event & { data?: unknown }): void;
}

/**
 * WebView2 augments the standard DOM event listener event with an additional data field. This field
 * contains unknown JSON encoded passed from the host via PostWebMessageAsJson.
...
export interface WebView extends EventTarget {
	 ...
	 * @param message The message to send to the WebView2 host. This can be unknown object that can be
	 *                serialized to JSON.
	 */
